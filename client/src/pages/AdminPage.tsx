...
export const AdminPage: React.FC = () => {
  ...
  const catalogValuation = products.reduce((acc, item) => acc + item.price * item.stock, 0);
  ...
  return (
    <div className="min-h-screen flex bg-transparent font-sans">
      {/* Sidebar for Desktop */}
      <aside className="hidden lg:flex flex-col w-64 bg-white/80 backdrop-blur-md border-r border-white/80 p-5 justify-between shadow-xs sticky top-0 h-screen">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 pb-4 border-b border-neutral-200/60">
            ...
            <div>
              <h2 className="font-black text-lg text-neutral-900 leading-tight">QuickCart</h2>
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition cursor-pointer ${
                activeTab === 'products'
                  ...
                  : 'text-neutral-700 hover:bg-white/90 hover:text-neutral-900'
              }`}
            >
              <span>📦</span>
        ...
        <div className="space-y-3 pt-4 border-t border-neutral-200/60">
          {user && (
            <div className="p-3 bg-white/70 rounded-xl border border-white/90 space-y-0.5">
              <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider block">
                Logged in Administrator
              </span>
              <span className="text-xs font-bold text-neutral-900 block truncate">{user.name}</span>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer active:scale-95 shadow-xs"
          >
            <span>🚪</span>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden bg-white/80 backdrop-blur-md border-b border-white/80 px-4 py-3 flex items-center justify-between sticky top-0 z-40">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-black text-sm">
              ⚡
            </div>
            <span className="font-black text-base text-neutral-900">QuickCart Admin</span>
          </div>

          <div className="flex items-center gap-2">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-2 rounded-xl bg-white text-neutral-700 border border-neutral-200 text-sm font-bold"
            >
              {sidebarOpen ? '✕' : '☰ Menu'}
            </button>
          </div>
        ...
        {/* Mobile Drawer Menu */}
        {sidebarOpen && (
          <div className="lg:hidden bg-white/95 backdrop-blur-lg border-b border-neutral-200 p-4 space-y-3 shadow-md">
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`py-2.5 px-3 rounded-xl text-xs font-bold text-center ${
                  activeTab === 'products' ? 'bg-indigo-600 text-white' : 'bg-neutral-100 text-neutral-800'
                }`}
              >
                📦 Products
              </button>
              <button
                ...
                className={`py-2.5 px-3 rounded-xl text-xs font-bold text-center ${
                  activeTab === 'overview' ? 'bg-indigo-600 text-white' : 'bg-neutral-100 text-neutral-800'
                }`}
              >
                📊 Overview
              </button>
            ...
            <button
              onClick={handleAdminLogout}
              className="w-full py-2.5 rounded-xl bg-rose-50 text-rose-700 border border-rose-200 text-xs font-bold text-center"
            >
              🚪 Admin Logout
            </button>
          </div>
        ...
        <main className="p-4 sm:p-6 lg:p-8 space-y-6 flex-1">
          ...
          {activeTab === 'products' && (
            <div className="space-y-6">
              ...
              <GlassCard className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/85">
                <div>
                  <h1 className="text-2xl sm:text-3xl font-black text-neutral-900 tracking-tight">
                    Product Inventory Management
                  </h1>
                  <p className="text-xs text-neutral-500 mt-1">
                    Create, update, and delete real MongoDB products.
                  </p>
                </div>
              ...
              <GlassCard className="p-4 flex items-center justify-between gap-4 bg-white/80">
                ...
                <span className="text-xs font-bold text-neutral-600 hidden sm:inline">
                  Filtered Items: <strong className="text-neutral-900">{filteredProducts.length}</strong>
                </span>
              </GlassCard>
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              <GlassCard className="p-6 bg-white/85">
                <h1 className="text-2xl sm:text-3xl font-black text-neutral-900 tracking-tight">
                  Inventory Overview & Analytics
                </h1>
                <p className="text-xs text-neutral-500 mt-1">
                  Key performance metrics for your MongoDB product catalog.
                </p>
              </GlassCard>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <GlassCard className="p-6 bg-white/80 space-y-2">
                  <span className="text-xs font-bold text-neutral-500 uppercase tracking-wider">
                    Total Products
                  </span>
                  <div className="text-3xl font-black text-neutral-900">{products.length}</div>
                </GlassCard>
                ...
                <GlassCard className="p-6 bg-white/80 space-y-2">
                  ...
                  <div className="text-3xl font-black text-indigo-600">{categoriesCount}</div>
                </GlassCard>
                ...
                <GlassCard className="p-6 bg-white/80 space-y-2">
                  ...
                  <div className={`text-3xl font-black ${outOfStockCount > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                    {outOfStockCount}
                  </div>
                </GlassCard>
                ...
                <GlassCard className="p-6 bg-white/80 space-y-2">
                  ...
                  <div className="text-3xl font-black text-neutral-900">₹{catalogValuation.toLocaleString()}</div>
                </GlassCard>
              </div>
            </div>
          )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-900/40 backdrop-blur-xs">
          <div className="w-full max-w-lg bg-white/95 backdrop-blur-md rounded-2xl border border-white p-6 shadow-xl space-y-5">
